...
function void update_animation_player(GameState* game_state,  V2_S32 ddp, SimEntity* entity, LowEntity* low, B32 force = false){
  ...
  if(ddp.x != 0 || ddp.y != 0){
    ...
    if(!(is_flag_set(tile, tile_occoupied) || is_flag_set(tile, tile_path)) || force){
        ...
        if(is_flag_set(source_tile, tile_entity)){
          ...
          for(int i = 0; i < array_count(top->banners); i++){
            ...
            if(btile->status != banner_status_empty){
              ...
              LowEntity* banner_entity = &game_state->low_entities[btile->low_index];
              Tile* entity_tile = get_tile(game_state->world, banner_entity->pos);
...
function void undo_player(GameState*  game_state, LowEntity* low_entity, V2_S32* ddp){
            ...
            switch (entity->type) {
              ...
              case entity_type_grass:{
                ...
                if(game_state->player_index > 0){
                  ...
                  S32 len = get_path_length(player->sim.tile_path);
                  ...
                  if(len >= 10){
                    ...
                    while(len != 0){
                      ...
                      LoadedBitmap* bmp = get_font_number(&game_state->font_asset, a);
                      opengl_bitmap(bmp, min_x - (i-1) * mtop, min_y,  width_pixels, height_pixels);
                      len = len /10;
                      i+=1;
                    }

                  }else{
                    LoadedBitmap* bmp= get_font_number(&game_state->font_asset, len);
                    opengl_bitmap(bmp, min_x, min_y,  width_pixels, height_pixels);
                  }
                }
                */
                }
...
function void render_game(OffscreenBuffer* buffer, PlatformState* platform_state, GameInput* input) {
  ...
  for (S32 i = 0; i < array_count(input->controllers); i += 1) {
    ...
    if (chunk->player_index == 0) {
      ...
      if (controller->is_analog) {
        ...
        if (entity->type == entity_type_player) {
          //Player movement
          if (ended_down(move_up, controller))  player_ddp.y = 1;
  ...
  //update_animation_chunk(game_state, &game_state->chunk_animation, camera_ddp, &game_state->camera_p);
